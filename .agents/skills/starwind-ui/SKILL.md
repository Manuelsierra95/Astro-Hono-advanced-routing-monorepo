...
- `starwind.config.json` -> `starwind.css`, `componentDir`, base color, and installed component list. In this repo, `componentDir` resolves to `packages/ui/src/components` and component imports should use `@workspace/ui/components/<component-name>`.
- Active Starwind CSS file -> global tokens, `@theme inline`, `:root`, `.dark`, and sidebar tokens.
- Installed component directory, commonly `packages/ui/src/components` here -> available imports and local source.
...
pnpx starwind@latest init --defaults
pnpx starwind@latest init --pro
import CardContent from "@workspace/ui/components/card-content";
import CardHeader from "@workspace/ui/components/card-header";
import CardTitle from "@workspace/ui/components/card-title";
pnpx starwind@latest add button
pnpx starwind@latest add button card select
pnpx starwind@latest add --all
pnpx starwind@latest add button --yes
pnpx starwind@latest add button --package-manager pnpm
pnpx starwind@latest add @starwind-pro/hero-01 button
...
```bash
pnpx starwind@latest docs button dialog select --json
```
...
4. **Get docs and examples** -> use Starwind MCP or `pnpx starwind@latest docs <component> --json`, then fetch the docs content.
...
# Add components
pnpx starwind@latest add button card dialog
pnpx starwind@latest add select
pnpx starwind@latest add theme-toggle

# Add all components
pnpx starwind@latest add --all

# Search components and Pro blocks
pnpx starwind@latest search select
pnpx starwind@latest search hero --json
pnpx starwind@latest search pricing --plan free --category pricing --limit 10 --json

# Get docs references
pnpx starwind@latest docs button dialog select --json

# Configure Starwind Pro in an existing project
pnpx starwind@latest setup
pnpx starwind@latest setup --yes
