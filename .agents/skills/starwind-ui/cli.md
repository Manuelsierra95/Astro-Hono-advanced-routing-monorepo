...
> **IMPORTANT:** Run commands from the user's project root. Examples below use `pnpx starwind@latest` for this workspace, but substitute the project's package runner when it is clear: `npx starwind@latest`, `pnpx starwind@latest` / `pnpm dlx starwind@latest`, or `yarn dlx starwind@latest`.
...
```bash
pnpx starwind@latest init [options]
```
...
```bash
pnpx starwind@latest add [components...] [options]
```
...
```bash
pnpx starwind@latest update [components...] [options]
```
...
```bash
pnpx starwind@latest update button
pnpx starwind@latest update button card --yes
pnpx starwind@latest update --all
```
...
- Pro blocks may require `pnpx starwind@latest setup` before installation.

### `docs` - Get component documentation references

```bash
pnpx starwind@latest docs [components...] [options]
```
...
```bash
pnpx starwind@latest docs button
pnpx starwind@latest docs button card --json
```
...
```bash
pnpx starwind@latest setup [options]
```
...
pnpx starwind@latest setup --package-manager pnpm
```
...
- For docs, use Starwind MCP, `pnpx starwind@latest docs <component> --json`, Starwind Markdown docs, and local installed source.
